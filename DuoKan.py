...
class DuoKan:
    ...
    def gift(self, cookies):
        ...
        if result.get("chances") == 0:
            ...
            for i in range(len(self.gift_code_list)-result.get("chances"), len(self.gift_code_list)):
                url = "https://www.duokan.com/events/common_task_gift"
                data = f"code=KYKJF7LL0G&chances=1&sign={self.gift_code_list[i]}&{self.get_data(cookies=cookies)}&withid=1"
                response = requests.post(
                    url=url, data=data, cookies=cookies, headers=self.headers)
                result = response.json()
...
if __name__ == '__main__':
    ...
    Cookies = config.get('DuoKan')
    if Cookies != None:
        if Cookies.get('cookies') != None:
            duokan = DuoKan(Cookies['cookies'])
            sio = duokan.SignIn()
            print(f'\n{sio.getvalue()}')
            if Cookies.get('send') != None and Cookies['send'] == 1:
                send('多看阅读', sio.getvalue())
            else:
                print('推送失败: 关闭了推送 or send配置问题')
        else:
            print('配置文件 多看阅读 没有 "cookies"')
            sys.exit()
    else:
        print('配置文件没有 多看阅读')